...
new #[Layout('layouts.dashboard')] #[Title('Penimbang')] class extends Component {
    use WithPagination;
    ...
    public array $beratAwal = [];

    /** @var array<int, int|string> */
    public array $beratDaging = [];

    /** @var array<int, int|string> */
    public array $beratTulang = [];
    ...
    public function saveWeights(int $hewanId): void
    {
        $hewan = Hewan::query()->findOrFail($hewanId);

        $hewan->update([
            'berat_awal' => max(0, (int) ($this->beratAwal[$hewan->id] ?? 0)),
            'berat_daging' => max(0, (int) ($this->beratDaging[$hewan->id] ?? 0)),
            'berat_tulang' => max(0, (int) ($this->beratTulang[$hewan->id] ?? 0)),
        ]);

        Flux::toast(variant: 'success', text: __('Berat hewan berhasil diperbarui.'));
    }
    ...
    public function with(): array
    {
        ...
        foreach ($hewanItems as $hewan) {
            $this->beratAwal[$hewan->id] = $this->beratAwal[$hewan->id] ?? $hewan->berat_awal;
            $this->beratDaging[$hewan->id] = $this->beratDaging[$hewan->id] ?? $hewan->berat_daging;
            $this->beratTulang[$hewan->id] = $this->beratTulang[$hewan->id] ?? $hewan->berat_tulang;
        }

        return [
            'pageTitle' => __('Penimbang'),
...
<section class="w-full space-y-6 p-6" wire:poll.10s>
    <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
            ...
            <flux:subheading>{{ __('Input dan edit berat hewan secara realtime.') }}</flux:subheading>
        </div>
        ...
        @forelse ($hewanItems as $hewan)
            <article class="space-y-3 rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                ...
                <div class="grid grid-cols-1 gap-2 sm:grid-cols-3">
                    <flux:input type="number" wire:model="beratAwal.{{ $hewan->id }}" min="0" :label="__('Berat Awal')" />
                    <flux:input type="number" wire:model="beratDaging.{{ $hewan->id }}" min="0" :label="__('Berat Daging')" />
                    <flux:input type="number" wire:model="beratTulang.{{ $hewan->id }}" min="0" :label="__('Berat Tulang')" />
                </div>

                <flux:button size="sm" variant="primary" class="w-full" wire:click="saveWeights({{ $hewan->id }})">{{ __('Simpan') }}</flux:button>
            </article>
        @empty
    ...
    <div class="hidden md:block">
        <flux:table :paginate="$hewanItems">
            ...
            <flux:table.rows>
                @forelse ($hewanItems as $hewan)
                    <flux:table.row :key="'penimbang-hewan-'.$hewan->id">
                        ...
                        <flux:table.cell><flux:input type="number" wire:model="beratAwal.{{ $hewan->id }}" min="0" class="w-24" /></flux:table.cell>
                        <flux:table.cell><flux:input type="number" wire:model="beratDaging.{{ $hewan->id }}" min="0" class="w-24" /></flux:table.cell>
                        <flux:table.cell><flux:input type="number" wire:model="beratTulang.{{ $hewan->id }}" min="0" class="w-24" /></flux:table.cell>
                        <flux:table.cell><flux:button size="sm" variant="primary" wire:click="saveWeights({{ $hewan->id }})">{{ __('Simpan') }}</flux:button></flux:table.cell>
                    </flux:table.row>
                @empty
                    <flux:table.row>
