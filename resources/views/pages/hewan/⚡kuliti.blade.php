...
new #[Layout('layouts.dashboard')] #[Title('Kuliti')] class extends Component {
    use WithPagination;
    ...
    public function startKuliti(int $hewanId): void
    {
        $hewan = Hewan::query()->findOrFail($hewanId);

        if ($hewan->selesai_jagal === null || $hewan->mulai_kuliti !== null) {
            return;
        }

        $hewan->update(['mulai_kuliti' => now()]);
        Flux::toast(variant: 'success', text: __('Tahap kuliti berhasil dimulai.'));
    }

    public function finishKuliti(int $hewanId): void
    {
        $hewan = Hewan::query()->findOrFail($hewanId);

        if ($hewan->mulai_kuliti === null || $hewan->selesai_kuliti !== null) {
            return;
        }

        $hewan->update(['selesai_kuliti' => now()]);
        Flux::toast(variant: 'success', text: __('Tahap kuliti berhasil diselesaikan.'));
    }
    ...
    public function with(): array
    {
        return [
            ...
            'hewanItems' => Hewan::query()
                ...
                ->orderByRaw("CASE WHEN mulai_kuliti IS NOT NULL AND selesai_kuliti IS NULL THEN 0 WHEN mulai_kuliti IS NULL THEN 1 ELSE 2 END")
                ->orderByRaw("CASE WHEN mulai_kuliti IS NOT NULL AND selesai_kuliti IS NULL THEN mulai_kuliti END ASC")
                ->orderByRaw("CASE WHEN mulai_kuliti IS NULL THEN id END DESC")
                ->orderByRaw("CASE WHEN selesai_kuliti IS NOT NULL THEN selesai_kuliti END DESC")
                ->paginate(12),
...
<section class="w-full space-y-6" wire:poll.10s x-data="dashboardDurations">
    <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
            ...
            <flux:subheading>{{ __('Pantau dan update proses kuliti secara realtime.') }}</flux:subheading>
        </div>
        ...
        @forelse ($hewanItems as $hewan)
            <article class="space-y-3 rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                ...
                <div class="flex items-center justify-between gap-2">
                    ...
                    <flux:text class="font-medium" x-text="stageDurationValue({{ $hewan->mulai_kuliti?->timestamp ?? 'null' }}, {{ $hewan->selesai_kuliti?->timestamp ?? 'null' }})">{{ $this->durationLabel($hewan->mulai_kuliti, $hewan->selesai_kuliti) }}</flux:text>
                </div>

                <div>
                    @if ($hewan->mulai_kuliti === null)
                        <flux:button size="sm" class="w-full" wire:click="openConfirmModal({{ $hewan->id }}, 'startKuliti', 'Mulai Kuliti', 'Yakin ingin memulai proses kuliti?')">{{ __('Mulai') }}</flux:button>
                    @elseif ($hewan->selesai_kuliti === null)
                        <flux:button size="sm" variant="primary" class="w-full" wire:click="openConfirmModal({{ $hewan->id }}, 'finishKuliti', 'Selesaikan Kuliti', 'Yakin ingin menyelesaikan proses kuliti?')">{{ __('Selesai') }}</flux:button>
                    @endif
                </div>
            </article>
        @empty
    ...
    <div class="hidden md:block">
        <flux:table :paginate="$hewanItems">
            ...
            <flux:table.rows>
                @forelse ($hewanItems as $hewan)
                    <flux:table.row :key="'kuliti-hewan-'.$hewan->id">
                        ...
                        <flux:table.cell x-text="stageDurationValue({{ $hewan->mulai_kuliti?->timestamp ?? 'null' }}, {{ $hewan->selesai_kuliti?->timestamp ?? 'null' }})">{{ $this->durationLabel($hewan->mulai_kuliti, $hewan->selesai_kuliti) }}</flux:table.cell>
                        <flux:table.cell>
                            @if ($hewan->mulai_kuliti === null)
                                <flux:button size="sm" wire:click="openConfirmModal({{ $hewan->id }}, 'startKuliti', 'Mulai Kuliti', 'Yakin ingin memulai proses kuliti?')">{{ __('Mulai') }}</flux:button>
                            @elseif ($hewan->selesai_kuliti === null)
                                <flux:button size="sm" variant="primary" wire:click="openConfirmModal({{ $hewan->id }}, 'finishKuliti', 'Selesaikan Kuliti', 'Yakin ingin menyelesaikan proses kuliti?')">{{ __('Selesai') }}</flux:button>
                            @endif
                        </flux:table.cell>
                    </flux:table.row>
                @empty
