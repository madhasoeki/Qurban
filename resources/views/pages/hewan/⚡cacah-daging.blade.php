    ...
    public function with(): array
    {
        return [
            ...
            'hewanItems' => Hewan::query()
                ...
                ->orderByRaw("CASE WHEN mulai_cacah_daging IS NOT NULL AND selesai_cacah_daging IS NULL THEN 0 WHEN mulai_cacah_daging IS NULL THEN 1 ELSE 2 END")
                ->orderByRaw("CASE WHEN mulai_cacah_daging IS NOT NULL AND selesai_cacah_daging IS NULL THEN mulai_cacah_daging END ASC")
                ->orderByRaw("CASE WHEN mulai_cacah_daging IS NULL THEN id END DESC")
                ->orderByRaw("CASE WHEN selesai_cacah_daging IS NOT NULL THEN selesai_cacah_daging END DESC")
                ->paginate(12),
        ...
        @forelse ($hewanItems as $hewan)
            <article class="space-y-3 rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                ...
                <div class="flex items-center justify-between gap-2">
                    ...
                    <flux:text class="font-medium" x-text="stageDurationValue({{ $hewan->mulai_cacah_daging?->timestamp ?? 'null' }}, {{ $hewan->selesai_cacah_daging?->timestamp ?? 'null' }})">{{ $this->durationLabel($hewan->mulai_cacah_daging, $hewan->selesai_cacah_daging) }}</flux:text>
                </div>
            ...
            <flux:table.rows>
                @forelse ($hewanItems as $hewan)
                    <flux:table.row :key="'cacah-daging-hewan-'.$hewan->id">
                        ...
                        <flux:table.cell x-text="stageDurationValue({{ $hewan->mulai_cacah_daging?->timestamp ?? 'null' }}, {{ $hewan->selesai_cacah_daging?->timestamp ?? 'null' }})">{{ $this->durationLabel($hewan->mulai_cacah_daging, $hewan->selesai_cacah_daging) }}</flux:table.cell>
                        <flux:table.cell>
                            @if ($hewan->mulai_cacah_daging === null)
