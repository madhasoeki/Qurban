...
new #[Layout('layouts.dashboard')] #[Title('Jeroan')] class extends Component {
    use WithPagination;
    ...
    public function startJeroan(int $hewanId): void
    {
        $hewan = Hewan::query()->findOrFail($hewanId);

        if ($hewan->selesai_jagal === null || $hewan->mulai_jeroan !== null) {
            return;
        }

        $hewan->update(['mulai_jeroan' => now()]);
        Flux::toast(variant: 'success', text: __('Tahap jeroan berhasil dimulai.'));
    }

    public function finishJeroan(int $hewanId): void
    {
        $hewan = Hewan::query()->findOrFail($hewanId);

        if ($hewan->mulai_jeroan === null || $hewan->selesai_jeroan !== null) {
            return;
        }

        $hewan->update(['selesai_jeroan' => now()]);
        Flux::toast(variant: 'success', text: __('Tahap jeroan berhasil diselesaikan.'));
    }
    ...
    public function with(): array
    {
        return [
            ...
            'hewanItems' => Hewan::query()
                ...
                ->orderByRaw("CASE WHEN mulai_jeroan IS NOT NULL AND selesai_jeroan IS NULL THEN 0 WHEN mulai_jeroan IS NULL THEN 1 ELSE 2 END")
                ->orderByRaw("CASE WHEN mulai_jeroan IS NOT NULL AND selesai_jeroan IS NULL THEN mulai_jeroan END ASC")
                ->orderByRaw("CASE WHEN mulai_jeroan IS NULL THEN id END DESC")
                ->orderByRaw("CASE WHEN selesai_jeroan IS NOT NULL THEN selesai_jeroan END DESC")
                ->paginate(12),
...
<section class="w-full space-y-6" wire:poll.10s x-data="dashboardDurations">
    <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
            ...
            <flux:subheading>{{ __('Pantau dan update proses jeroan secara realtime.') }}</flux:subheading>
        </div>
        ...
        @forelse ($hewanItems as $hewan)
            <article class="space-y-3 rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                ...
                <div class="flex items-center justify-between gap-2">
                    ...
                    <flux:text class="font-medium" x-text="stageDurationValue({{ $hewan->mulai_jeroan?->timestamp ?? 'null' }}, {{ $hewan->selesai_jeroan?->timestamp ?? 'null' }})">{{ $this->durationLabel($hewan->mulai_jeroan, $hewan->selesai_jeroan) }}</flux:text>
                </div>

                <div>
                    @if ($hewan->mulai_jeroan === null)
                        <flux:button size="sm" class="w-full" wire:click="openConfirmModal({{ $hewan->id }}, 'startJeroan', 'Mulai Jeroan', 'Yakin ingin memulai proses jeroan?')">{{ __('Mulai') }}</flux:button>
                    @elseif ($hewan->selesai_jeroan === null)
                        <flux:button size="sm" variant="primary" class="w-full" wire:click="openConfirmModal({{ $hewan->id }}, 'finishJeroan', 'Selesaikan Jeroan', 'Yakin ingin menyelesaikan proses jeroan?')">{{ __('Selesai') }}</flux:button>
                    @endif
                </div>
            </article>
        @empty
    ...
    <div class="hidden md:block">
        <flux:table :paginate="$hewanItems">
            ...
            <flux:table.rows>
                @forelse ($hewanItems as $hewan)
                    <flux:table.row :key="'jeroan-hewan-'.$hewan->id">
                        ...
                        <flux:table.cell x-text="stageDurationValue({{ $hewan->mulai_jeroan?->timestamp ?? 'null' }}, {{ $hewan->selesai_jeroan?->timestamp ?? 'null' }})">{{ $this->durationLabel($hewan->mulai_jeroan, $hewan->selesai_jeroan) }}</flux:table.cell>
                        <flux:table.cell>
                            @if ($hewan->mulai_jeroan === null)
                                <flux:button size="sm" wire:click="openConfirmModal({{ $hewan->id }}, 'startJeroan', 'Mulai Jeroan', 'Yakin ingin memulai proses jeroan?')">{{ __('Mulai') }}</flux:button>
                            @elseif ($hewan->selesai_jeroan === null)
                                <flux:button size="sm" variant="primary" wire:click="openConfirmModal({{ $hewan->id }}, 'finishJeroan', 'Selesaikan Jeroan', 'Yakin ingin menyelesaikan proses jeroan?')">{{ __('Selesai') }}</flux:button>
                            @endif
                        </flux:table.cell>
                    </flux:table.row>
                @empty
